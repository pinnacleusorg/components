...
QUISP is versioned using Pinnacle's aligned semver system (major.minor.patch). The major version is aligned with the current year's event (e.g. v3.x.x correlates with Pinnacle 2023) and therefore also that year's branding guidelines. This allows for easy identification of project statuses along with their corresponding event year.
